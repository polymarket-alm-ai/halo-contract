...
import { depositToAster, withdrawFromAster, returnToVault } from './asterDexHelper';
...
async function main() {
  ...
  if (!command) {
    console.log('Usage:');
    console.log('  npm run agent:aster deposit <amount>  - Deposit USDT to Aster DEX');
    console.log('  npm run agent:aster withdraw <amount> - Withdraw USDT from Aster DEX');
    console.log('  npm run agent:aster return <amount>   - Return USDT from EOA to vault');
  ...
  switch (command) {
    ...
    case 'withdraw': {
      ...
      await withdrawFromAster(amount, agentSigner, ASTER_API_KEY, ASTER_API_SECRET);
    ...
    case 'balance': {
      ...
      console.log(`Agent EOA Balance: ${ethers.formatEther(agentBalance)} USDT`);
