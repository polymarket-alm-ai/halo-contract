...
async function getVaultState() {
  const vault = await ethers.getContractAt('VaultERC4626v2', VAULT_ADDRESS);
...
function calculateBridgeAmount(
  ...
  console.log(`Bridge ${bridgePercent}% to Polygon: ${bridgeAmount.toFixed(2)} USDT`);
  console.log(`Keep ${100 - bridgePercent}% on BNB: ${keepAmount.toFixed(2)} USDT`);
  console.log('');
...
async function executeBridge(amount: number, dryRun: boolean = false) {
  ...
  console.log(`Gas fee required: ${ethers.formatEther(swapData.transactionRequest.value)} BNB`);

  // Execute bridge
  const vault = await ethers.getContractAt('VaultERC4626v2', VAULT_ADDRESS, signer);

  console.log('\nExecuting bridge transaction...');
  const tx = await vault.bridgeToPolygon(amountWei, swapData.transactionRequest.data, {
...
async function main() {
  ...
  const strategy = strategyArg
    ? (strategyArg.split('=')[1] as 'conservative' | 'balanced' | 'aggressive' | 'custom')
    : 'balanced';
  ...
  if (amountArg) {
    ...
    // Use strategy
    const customPercent = strategy === 'custom' && percentArg
      ? parseFloat(percentArg.split('=')[1])
      : undefined;
    bridgeAmount = calculateBridgeAmount(state.vaultBalance, strategy, customPercent);
  }

  // Validate amount
  if (bridgeAmount > state.vaultBalance) {
    console.error(`❌ Error: Bridge amount (${bridgeAmount}) exceeds vault balance (${state.vaultBalance})`);
    return;
  }
